...
test.describe('File Menu Tests', () => {
  ...
  test('File menu exists with all required items', async ({
    electronApp,
    window,
  }) => {
    ...
    const menuItems = menuStructure.items.filter(
  ...
  test('Menu items have correct keyboard shortcuts', async ({
    electronApp,
    window,
  }) => {
      ...
      shortcuts = await electronApp.evaluate(({ Menu }) => {
        ...
        if (!fileMenu)
          return {
            ...
            menus: appMenu.items.map((i) => i.label),
          };
  ...
  test('Save menu item is initially disabled', async ({
    electronApp,
    window,
  }) => {
    // Wait for app to be ready
    ...
    for (let i = 0; i < 20; i++) {
      saveEnabled = await electronApp.evaluate(({ Menu }) => {
        ...
        if (!saveItem)
          return {
            error: 'Save menu item not found',
  ...
  test('Recent Files submenu exists', async ({ electronApp, window }) => {
    ...
    for (let i = 0; i < 20; i++) {
      recentFilesExists = await electronApp.evaluate(({ Menu }) => {
        ...
        const recentItem = fileMenu.submenu.items.find(
          (item) => item.label === 'Open Recent'
        );
        if (!recentItem)
          return {
            error: 'Open Recent not found',
            items: fileMenu.submenu.items.map((i) => i.label),
          };
        ...
        return {
          ...
          submenuItems: recentItem.submenu
            ? recentItem.submenu.items.map((i) => i.label)
            : [],
        };
      });
    ...
    expect(
      recentFilesExists.submenuItems.some(
        (l) => l === 'No Recent Files' || l === 'Clear Recent'
      )
    ).toBe(true);
  });

  test('Menu items can be accessed by ID programmatically', async ({
    electronApp,
    window,
  }) => {
    // Wait for app to be ready
    ...
    for (let i = 0; i < 20; i++) {
      ...
      if (
        menuItemsAccessible &&
        !menuItemsAccessible.error &&
        menuItemsAccessible.results &&
        menuItemsAccessible.results['new']
      ) {
        break;
      }
      await window.waitForTimeout(100);
  ...
  test('Menu triggers correct IPC events', async ({ electronApp }) => {
  ...
  test('Menu state updates when file is modified', async ({
    electronApp,
    window,
  }) => {
    // Wait for app to be ready
...
test.describe('Menu Integration Tests', () => {
  ...
  test('New menu item triggers new file action', async ({
    electronApp,
    window,
  }) => {
    await window.waitForLoadState('domcontentloaded');
