...
test.describe('Edit Menu Tests', () => {
  ...
  test('Edit menu exists with all required items', async ({ electronApp, window }) => {
    ...
    console.log('[Edit Menu Test] Found', editMenuItems.length, 'Edit menu items');
    ...
    const selectAllItem = editMenuItems.find(item => item.label === 'Select All');
    expect(selectAllItem).toBeTruthy();
    expect(selectAllItem.accelerator).toContain('A');
    
    // Check for separator before Select All
    const separatorIndex = editMenuItems.findIndex(item => item.type === 'separator');
  ...
  test('Copy menu item copies cell value to clipboard', async ({ electronApp, window }) => {
    await window.waitForLoadState('domcontentloaded');
    ...
    await electronApp.evaluate(({ Menu }) => {
      const menu = Menu.getApplicationMenu();
      const editMenu = menu.items.find(item => item.label === 'Edit');
      const copyItem = editMenu.submenu.items.find(item => item.label === 'Copy');
    ...
    await electronApp.evaluate(({ Menu }) => {
      const menu = Menu.getApplicationMenu();
      const editMenu = menu.items.find(item => item.label === 'Edit');
      const pasteItem = editMenu.submenu.items.find(item => item.label === 'Paste');
  ...
  test('Cut menu item cuts cell value', async ({ electronApp, window }) => {
    ...
    await electronApp.evaluate(({ Menu }) => {
      const menu = Menu.getApplicationMenu();
      const editMenu = menu.items.find(item => item.label === 'Edit');
      const cutItem = editMenu.submenu.items.find(item => item.label === 'Cut');
  ...
  test('Paste menu item pastes clipboard content', async ({ electronApp, window }) => {
    await window.waitForLoadState('domcontentloaded');
  ...
  test('Keyboard shortcuts are registered for Edit menu', async ({ electronApp, window }) => {
    await window.waitForLoadState('domcontentloaded');
    ...
    const shortcuts = await electronApp.evaluate(({ Menu }) => {
      ...
      return {
        cut: editMenu.submenu.items.find(item => item.label === 'Cut')?.accelerator,
        copy: editMenu.submenu.items.find(item => item.label === 'Copy')?.accelerator,
        paste: editMenu.submenu.items.find(item => item.label === 'Paste')?.accelerator,
        selectAll: editMenu.submenu.items.find(item => item.label === 'Select All')?.accelerator
      };
    });
    ...
    console.log('[Edit Menu Test] All keyboard shortcuts are registered:', shortcuts);
  });
  ...
  test('Select All menu item works', async ({ electronApp, window }) => {
    ...
    await electronApp.evaluate(({ Menu }) => {
      const menu = Menu.getApplicationMenu();
      const editMenu = menu.items.find(item => item.label === 'Edit');
      const selectAllItem = editMenu.submenu.items.find(item => item.label === 'Select All');
