...
// Detect if we're running in Wails or web demo
const isWails = typeof window.go !== 'undefined';

let GetCellValue, GetCellRawValue, SetCellValue, GetCellRef, GetAllCells;

if (isWails) {
    // Use Wails bindings (dynamic import will be handled by Vite)
    try {
        const wailsBindings = require('../wailsjs/go/main/App');
        GetCellValue = wailsBindings.GetCellValue;
        GetCellRawValue = wailsBindings.GetCellRawValue;
        SetCellValue = wailsBindings.SetCellValue;
        GetCellRef = wailsBindings.GetCellRef;
        GetAllCells = wailsBindings.GetAllCells;
        console.log('Using Wails bindings');
    } catch (e) {
        console.warn('Wails bindings not available:', e);
    }
}

if (!GetCellValue) {
    // Use HTTP API for web demo
    console.log('Using HTTP API (web demo mode)');
    
    GetCellValue = async (row, col) => {
        const response = await fetch(`/api/GetCellValue?row=${row}&col=${col}`);
        return await response.json();
    };
    
    GetCellRawValue = async (row, col) => {
        const response = await fetch(`/api/GetCellRawValue?row=${row}&col=${col}`);
        return await response.json();
    };
    
    SetCellValue = async (row, col, value) => {
        const response = await fetch('/api/SetCellValue', {
            method: 'POST',
    ...
    GetCellRef = async (row, col) => {
        const response = await fetch(`/api/GetCellRef?row=${row}&col=${col}`);
        return await response.json();
    };
    
    GetAllCells = async () => {
        const response = await fetch('/api/GetAllCells');
        return await response.json();
    };
}
...
function startEditingWithChar(row, col, initialChar) {
    ...
    isEditing = true;
    console.log(`Starting edit with char "${initialChar}" at row=${row}, col=${col}, isEditing=${isEditing}`);
    selectCell(row, col);
